    ...
    >
      ...
      <div className="relative z-10 flex items-start justify-between">
        <div className="space-y-1.5">
          <p className={`text-[11px] font-semibold uppercase tracking-widest ${accent ? 'text-primary-foreground/60' : 'text-muted-foreground'}`}>
            {title}
          </p>
          ...
          {description && (
            <p className={`text-xs font-medium ${accent ? 'text-primary-foreground/50' : 'text-muted-foreground'}`}>
              {description}
            </p>
          )}
        </div>
        <div className={`p-2.5 rounded-xl ${accent ? 'bg-white/10 border border-white/10' : 'bg-primary/5 border border-primary/10'}`}>
